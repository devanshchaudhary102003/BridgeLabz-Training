...
namespace AddressBook
{
    public class AddressBookUtility : IAddressBook
    {
        ...
        public async Task GetAllPersonsFromJsonServer()
        {
            ...
            List<AddressBook> contacts = JsonConvert.DeserializeObject<List<AddressBook>>(jsonResponse);
        ...
        //UC 17 UPDATE OPERATION
        public async Task UpdatePersonInJsonServerAsync(int id, AddressBook updatedPerson)
        {
            string json = JsonConvert.SerializeObject(updatedPerson);
            StringContent content = new StringContent(json, Encoding.UTF8, "application/json");

            HttpResponseMessage response = await httpClient.PutAsync($"http://localhost:3000/contacts/{id}", content);
        ...
        //UC 17 DELETE OPERATION
        public async Task DeletePersonFromJsonServerAsync(int id)
        {
            HttpResponseMessage response = await httpClient.DeleteAsync($"http://localhost:3000/contacts/{id}");
        ...
        public void WriteAddressBookToJsonFile()
        {
            ...
            string jsonData = System.Text.Json.JsonSerializer.Serialize(addressBooks[book], options);
        ...
        public void ReadAddressBookFromJsonFile()
        {
            ...
            List<AddressBook> person = System.Text.Json.JsonSerializer.Deserialize<List<AddressBook>>(jsonData);
        ...
        // UC - 17 Write Address Book To Text File
        public async Task WriteAddressBookToFileAsync()
        {
            ...
            foreach(var person in addressBooks[book])
            {  
                // Append person details in pipe-separated format
                builder.AppendLine($"{person.firstName}|{person.lastName}|{person.address}|{person.city}|{person.state}|{person.zip}|{person.phoneNumber}|{person.email}");
            }

            // Asynchronously write data to text file (non-blocking I/O)
            await File.WriteAllTextAsync(file,builder.ToString());
            Console.WriteLine("Address book written to text file successfully");
        }

        //UC - 17 Read Address Book From Text File
        public async Task ReadAddressBookFromFileAsync()
        {
            ...
            Console.WriteLine("Address book read from text file successfully");
        }

        //UC - 17 Write Address Book To CSV File
        public async Task WriteAddressToCsvFileAsync()
        {
            Console.WriteLine("Enter Address Book Name: ");
            string book = Console.ReadLine();
            ...
            string file = book + ".csv";

            await using var writer = new StreamWriter(file);
            await using var csv = new CsvWriter(writer,CultureInfo.InvariantCulture);

            await csv.WriteRecordsAsync(addressBooks[book]);
            Console.WriteLine("Address book written to CSV file successfully");
        }

        //UC - 17 Read Address Book From CSV File
        public async Task ReadAddressFromCsvFileAsync()
        {
            Console.WriteLine("Enter Address Book Name: ");
            string book = Console.ReadLine();
            ...
            await foreach(var record in csv.GetRecordsAsync<AddressBook>())
            {
                records.Add(record);
            }
            ...
            addressBooks[book].AddRange(records);

            Console.WriteLine("Address book read from CSV file successfully");
        }

        //UC - 17 Write Address Book To JSON File
        public async Task WriteAddressToJsonFileAsync()
        {
            Console.WriteLine("Enter Address Book Name: ");
            string book = Console.ReadLine();
        ...
        //UC - 17 Read Address Book From JSON File
        public async Task ReadAddressFromJsonFileAsync()
        {
            Console.WriteLine("Enter Address Book Name: ");
            string book = Console.ReadLine();
            ...
            List<AddressBook> persons = System.Text.Json.JsonSerializer.Deserialize<List<AddressBook>>(jsonData);
